...
from home.forms import PostsForm, RegistrationForm, LoginForm
from home.models import Post, Like, Dislike
...
def like_view(request):
    ...
    if not likes:
        if dislikes:
            dislike = Dislike.objects.get(post=request.POST.get('post_id'),
                                    user=request.user)
            dislike.delete()
            post.dislikes -= 1
...
def dislike_view(request):
    ...
    if not dislikes:
        if likes:
            like = Like.objects.get(post=request.POST.get('post_id'),
                                user=request.user)
            like.delete()
            post.likes -= 1
